...
update() {
  ...
  const shouldRender = delta > 0.001; // Render only if scroll changed enough

  if (this.medias && shouldRender) {
    this.medias.forEach((media) => {
      media.update(this.scroll, direction);

      // Only update time if scroll is active (for performance)
      if (Math.abs(media.speed) > 0.001) {
